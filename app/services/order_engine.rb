class OrderEngine
   ...
   def self.order(order_id, user_id, menu_item_id, qty)
     order = find_or_create_order(order_id, user_id)
     
     order_item = update_or_create_order_item(order, qty, menu_item_id)
   ...
   def self.update_or_create_order_item(order, quantity, menu_id)
     order_item = order.order_items.find_by(menu_item_id: menu_id)
